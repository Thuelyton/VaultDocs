/**
 * OpenAI AI Provider
 * Uses OpenAI GPT for document data extraction
 */
 ...
 */
const OPENAI_CONFIG = {
  model: 'gpt-4o-mini', // Cost-effective for document extraction
...
- person: Person information (name, cpf, rg, birthDate, etc.)
- address: Address if available
- issueDate: Issue date
- expirationDate: Expiration date
- issuer: Issuing authority
- amount: Monetary amount if applicable
- dueDate: Due date for bills
    ...
    try {
      const response = await this.client!.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: `Classify the document type from the OCR text. 
